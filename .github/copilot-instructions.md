...
Before running any Azure CLI command in the Pre-Test Checklist steps, verify you are logged into the correct tenant and subscription. Each developer maintains their own values locally — never commit real values to this file. See README 'Running Locally' for setup.
...
- **BYOK token lifecycle (critical)**: `ProviderConfig.BearerToken` is a **static string baked into the Copilot CLI subprocess at session creation** — there is no callback/delegate to push refreshed tokens into a running session. AOAI tokens last ~1h, so any `CopilotSession` older than its token's expiry will fail every prompt with `HTTP 401 Authentication failed with provider`. `CopilotSessionFactory` mitigates this by tracking `LiveSessionInfo.BearerExpiry` (per live session, stored inside the `LiveSessions` dict in `AiTelemetry`) and **proactively recycling via `ResumeSessionAsync` with a fresh token** in `GetOrCreateSessionAsync` when `expiry - now < 10 min` — preserves history. If you ever swap providers or change the BYOK credential type, preserve this proactive-recycle pattern — refreshing only the cached token string is NOT enough. If `ResumeSessionAsync` throws during proactive recycle, fall back to creating a new session via the normal creation path and log the failure as a warning to Application Insights. Do not surface the error to the user as a hard failure.
...
| Cost Allocation      | `User.Read.All`, `Group.Read.All`           | Read-only by scope definition    | Cannot modify users or groups; `QueryGraph` allows GET/POST/PUT/PATCH — DELETE blocked in code |
...
- **Scheduled Jobs** (`Jobs/JobStore.cs`, `Jobs/JobScheduler.cs`, `Jobs/JobEndpoints.cs`) — a user-defined prompt + cadence executed as a background agent turn in its own dedicated Copilot session (which doubles as the job's run history).

  **Cadence & Caps**
  - Presets: 15 / 60 / 1440 / 10080 min; or any custom value from 1 min to 43200 min (30 days) — validated in the modal (create/save disables + inline hint) and by the API (`MinIntervalMinutes=1`, `MaxIntervalMinutes=43200` in `JobEndpoints`).
  - Custom cadences label in the largest whole unit (`every N min` / `every N h` / `every N d`).
  - 1 min = the scheduler's tick cadence (fastest it can fire); runs never overlap thanks to the per-session turn gate.
  - Caps: 3 active jobs/user; sub-daily expire 7 d, daily/weekly 90 d; auto-pause after 5 consecutive failures or `auth_expired`.

  **Auth & Token Lifecycle**
  - Entra-only; delegated-only auth — each run re-mints tokens from the persisted MSAL refresh token via `SessionTokenStore.ExchangeRefreshTokenForResource`; no app permissions, capped at the user's RBAC.
  - Same tool surface as chat (writes allowed, DELETE blocked).

  **UI Behavior**
  - "Scheduled jobs" pane below Conversations in `ChatView.vue` (top pane titled "Agent execution").
  - "+ New job" opens a centered modal sheet (teleported to body, blurred backdrop, Esc/backdrop close) with one-click templates, cadence pills, and a "Run immediately" checkbox (default on). Templates: "Check capacity of X" (read-only 15-min hunt), "Reserve X when available" (ARM PUT, idempotent, cost-warned), "1-min test" (one-sentence USD report), plus cost digest/anomaly/budget/idle/Advisor.
  - Rows auto-sort: running → failing → soonest next run → paused; live countdowns (30 s ticker); animated status dots (blue breathing = armed, green pulse = running, red pulse = failed); header shows "N active · M failing"; failed last run surfaces as "failed Xm ago · next …" in red.
  - Row actions (▶ run-now, ✎ edit, on/off switch, × delete) are a hover/focus-revealed OVERLAY on the right edge (absolute-positioned `.job-actions` with a gradient backdrop).
  - Each row has a pencil (✎) that reopens the modal in **edit mode** (prefilled name/prompt/cadence; template picker hidden; saves via `PATCH`; "Run now after saving" checkbox — default off). `openNewJob()` clears form state; all four close paths (Cancel/×/Esc/backdrop) reset edit state so a cancelled edit never leaks into a fresh job.
  - Clicking a job row opens its conversation as a RUN LOG (no composer; job bar offers Build deck / Summarize runs / Edit job; follow loop refreshes every 7 s while in view; sidebar title reads "⚙ Job · {name}").
  - Triggering a run (▶ or "Run now after saving") auto-opens the run's conversation via `watchJobRunAndOpen` (polls until session exists, opens it, refreshes transcript until run lands; background runs don't stream to the chat SSE; stops if the user navigates away).
  - Paused rows display "paused" — never a misleading next-run countdown.
  - Statuses refresh on tab-return + 45 s visible poll.
  - All three sidebar panes (Agent execution / Conversations / Jobs) collapse to their header via a chevron (sticky per pane in localStorage; collapsing two gives the third the whole sidebar).
  - Deletes (jobs AND conversations) are immediate + optimistic — no confirm dialogs; the row leaves before the server round-trip.

  **Session & Ownership Rules**
  - Shares the one-turn-per-session gate with chat (`ChatEndpoints.TryBeginTurn/EndTurn`); restores the user's `CurrentSessionId` around each run so a scheduled run never hijacks the active conversation.
  - Ownership is strictly OID-bound: `ResolveOwnedJob/ForUser/EnabledCountForUser` match `EntraOid` exactly (no userId-hash fallback); `JobScheduler` loads the identity record OID-first and refuses to run on record/job OID mismatch.
  - Job run logs are hidden from the Conversations pane (`chatSessions` filters sessions owned by live jobs); deleting the job un-hides its history.
  - `DELETE /api/sessions/{id}` calls `jobStore.DetachSession` so a deleted conversation never strands a job on a dead sessionId.
  - A job that hasn't run yet (no session) opens its edit/detail sheet instead of doing nothing.

  **Endpoints**
  - `GET/POST /api/jobs` — list / create.
  - `PATCH /api/jobs/{id}` — edit name/prompt/intervalMinutes (each optional; interval change recomputes `ExpiresUtc` + re-arms `NextRunUtc`).
  - `POST /api/jobs/{id}/run` — trigger immediate run.
  - `POST /api/jobs/{id}/toggle` — enable/disable; resume is cap-checked (re-enabling counts against the 3-active limit, so pause+create×3+resume can't yield 4 active).
...
3. **Confirm a browser page is shared** with you (look for the `Browser Pages` attachment in the conversation context). If none, call `open_browser_page url=http://localhost:5000` and capture the `pageId`. If `open_browser_page` returns an error or the page does not load within 10 seconds, STOP and report the failure with the error message. Do not proceed with the test steps.
